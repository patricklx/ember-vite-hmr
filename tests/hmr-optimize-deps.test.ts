...
// imports `@glimmer/reference`, `@glimmer/runtime` and `@ember/destroyable`.
// Those wrappers are generated/served on demand, so they are never in the
// static graph Vite's dependency scanner crawls on the first pass. Unless the
// plugin pre-declares them, the browser requests them on boot, Vite discovers
// "new" deps and triggers a re-optimize + full page reload.
//
